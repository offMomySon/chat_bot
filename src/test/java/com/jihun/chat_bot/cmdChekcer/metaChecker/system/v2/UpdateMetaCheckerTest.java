...
//    }
    ...
    void test6(String _meta) {
        String[] meta = _meta.split(" ");

        Queue<String> queue = new LinkedList<>(Arrays.asList(meta));

        UpdateMetaChecker defaultBannerUpdate = new UpdateMetaChecker(
            List.of(new TextMetaChecker(Collections.emptyList(), queue)), queue);

        UpdateMetaChecker fileUpdate = new UpdateMetaChecker(List.of(
            new FileMetaChecker(List.of(
                new TextMetaChecker(Collections.emptyList(), queue)
            ), queue)
        ), queue);

        UpdateMetaChecker exitUpdate = new UpdateMetaChecker(List.of(
            new ExitMetaChecker(List.of(
                new TextMetaChecker(Collections.emptyList(), queue)
            ), queue)
        ), queue);

        boolean isValid = false;
        List<AbstractMetaChecker> metaCheckers = List.of(defaultBannerUpdate, fileUpdate, exitUpdate);
        for (AbstractMetaChecker metaChecker : metaCheckers) {
            if (metaChecker.createErrorMsg() == MetaErrorMsg.EMPTY) {
                isValid = true;
                break;
            }
        }
